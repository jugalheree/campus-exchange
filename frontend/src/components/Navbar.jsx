...
export default function Navbar() {
  ...
  // ── Hide navbar on chat detail pages (they have their own header) ──
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-5 h-full flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            {user ? (
              <>
                ...
                <button onClick={() => setSidebarOpen(true)}
                  ...
                  <div className="relative">
                    <div className="w-7 h-7 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-xs font-bold overflow-hidden">
                      {user.avatar
              ...
              <>
                <Link to="/login" className="px-4 py-1.5 text-sm text-gray-400 hover:text-white transition">Login</Link>
                <Link to="/register" className="px-4 py-1.5 rounded-lg bg-white text-black text-sm font-semibold hover:bg-gray-100 transition">Sign up</Link>
              </>
            )}
      ...
      {sidebarOpen && <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />}
      ...
      <aside className={`fixed top-0 right-0 h-full w-72 z-[70] bg-[#0d0d0d] border-l border-white/[0.07] flex flex-col transition-transform duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] ${sidebarOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="flex items-center justify-between px-5 pt-5 pb-4 border-b border-white/[0.06] shrink-0">
          {user ? (
            <Link to="/profile" className="flex items-center gap-3 group" onClick={() => setSidebarOpen(false)}>
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white font-bold overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto py-3 px-3 space-y-0.5">
          ...
          {user && (
            <>
              <div className="pt-3 pb-1 px-2"><p className="text-[10px] text-gray-600 uppercase tracking-widest font-medium">Activity</p></div>
              ...
              <div className="pt-3 pb-1 px-2"><p className="text-[10px] text-gray-600 uppercase tracking-widest font-medium">You</p></div>
              ...
              <div className="pt-3 pb-1 px-2"><p className="text-[10px] text-gray-600 uppercase tracking-widest font-medium">Create</p></div>
          ...
          {!user && (
            <div className="pt-4 px-2 space-y-2">
              <Link to="/login" onClick={() => setSidebarOpen(false)} className="block w-full py-3 text-center rounded-xl border border-white/10 text-sm text-gray-300 hover:bg-white/5 transition">Login</Link>
              <Link to="/register" onClick={() => setSidebarOpen(false)} className="block w-full py-3 text-center rounded-xl bg-white text-black text-sm font-semibold hover:bg-gray-100 transition">Sign up</Link>
            </div>
          )}
        </div>
        ...
        {user && (
          <div className="shrink-0 border-t border-white/[0.06] p-3 space-y-0.5">
            <ThemeToggle />
            <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-500 hover:text-white hover:bg-white/5 transition">
...
function NavPill({ to, active, children }) {
  return <Link to={to} className={`px-3.5 py-1.5 rounded-lg text-sm transition-all ${active ? "text-white bg-white/10" : "text-gray-500 hover:text-white hover:bg-white/5"}`}>{children}</Link>;
}
function Dot({ n }) {
  return <span className="inline-flex items-center justify-center min-w-[15px] h-[15px] px-1 rounded-full bg-white text-black text-[9px] font-black">{n > 9 ? "9+" : n}</span>;
}
function SideItem({ to, icon, active, badge, children }) {
  return (
    <Link to={to} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all group ${active ? "bg-white/10 text-white" : "text-gray-400 hover:text-white hover:bg-white/[0.05]"}`}>
      <span className={`w-5 h-5 shrink-0 transition-colors ${active ? "text-white" : "text-gray-600 group-hover:text-gray-300"}`}>{icon}</span>
      <span className="flex-1">{children}</span>
      {badge > 0 && <Dot n={badge} />}
    </Link>
  );
}
const I = ({ d, ...p }) => <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8} {...p}><path strokeLinecap="round" strokeLinejoin="round" d={d} /></svg>;
