    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm rounded-t-3xl sm:rounded-3xl p-6"
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input type="text" placeholder="e.g. Parul University, GTU, PDPU..." value={university} onChange={e => setUniversity(e.target.value)} className="input-base" autoFocus />
...
export default function Register() {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.google) {
        window.google.accounts.id.initialize({ client_id: import.meta.env.VITE_GOOGLE_CLIENT_ID, callback: handleGoogleCallback });
      }
    };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-5 py-10" style={{ background: "#000", color: "#fff" }}>
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input type="text" placeholder="Full name" value={form.name} onChange={set("name")} className="input-base" required autoComplete="name" />
          <input type="email" placeholder="College email" value={form.email} onChange={set("email")} className="input-base" required autoComplete="email" />
          <input type="text" placeholder="University name" value={form.university} onChange={set("university")} className="input-base" required />
          <input type="password" placeholder="Password (min 6 characters)" value={form.password} onChange={set("password")} className="input-base" required minLength={6} autoComplete="new-password" />
        ...
        <p className="text-xs text-center mt-4" style={{ color: "#404040" }}>By signing up you agree to our terms of service</p>
        <div className="mt-5 text-center">
          <p className="text-sm" style={{ color: "#606060" }}>Already have an account?{" "}<Link to="/login" className="text-white font-semibold hover:opacity-80 transition">Sign in</Link></p>
        </div>
      </div>
    </div>
  );
