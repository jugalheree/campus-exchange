  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        if (p.seller?._id?.toString() !== user?.id) { navigate("/dashboard"); return; }
        setForm({ title: p.title, description: p.description, category: p.category, condition: p.condition, price: p.price.toString() });
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 pb-24">
        ...
        <form onSubmit={handleSubmit} className="card p-8 space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((cat) => (
                <button type="button" key={cat} onClick={() => setForm({ ...form, category: cat })}
                  className={`px-4 py-2 rounded-xl text-sm border transition ${form.category === cat ? "bg-white text-black border-white font-medium" : "border-white/10 text-gray-400 hover:border-white/20"}`}>
                  {cat}
                </button>
              ))}
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {CONDITIONS.map((c) => (
                <button type="button" key={c.value} onClick={() => setForm({ ...form, condition: c.value })}
                  className={`px-4 py-2 rounded-xl text-sm border transition ${form.condition === c.value ? "bg-white text-black border-white font-medium" : "border-white/10 text-gray-400 hover:border-white/20"}`}>
                  {c.label}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input type="number" name="price" value={form.price} onChange={handleChange} className="input-base" required />
          </div>

          {existingImages.length > 0 && (
            <div>
              <label className="text-xs text-gray-500 uppercase tracking-wider mb-3 block">Current Images</label>
              <div className="flex gap-3 flex-wrap">
                {existingImages.map((img, i) => (
                  <img key={i} src={img} alt="" className="w-20 h-20 object-cover rounded-xl opacity-60" />
                ))}
              </div>
            </div>
          )}
          ...
          <div>
            <label className="text-xs text-gray-500 uppercase tracking-wider mb-3 block">Replace Images (optional)</label>
            <label className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-xl p-6 cursor-pointer hover:border-white/20 transition">
              <span className="text-gray-500 text-sm">Click to upload new images</span>
            ...
            {previews.length > 0 && (
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 mt-3">
                {previews.map((src, i) => <img key={i} src={src} alt="" className="rounded-xl h-20 w-full object-cover" />)}
              </div>
            )}
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Link to={`/products/${id}`} className="btn-ghost px-6 py-3.5 rounded-xl text-sm">Cancel</Link>
          </div>
        </form>
      </div>
    </div>
